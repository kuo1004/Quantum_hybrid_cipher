...
from database import connect_db, insert_image_upload, get_image_data, init_database
...
def health_check():
    ...
    try:
        conn = connect_db()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM imageuploads")
...
def list_images():
    ...
    try:
        ...
        if patient_id:
            query = """
            SELECT id, patient_id, filename, mime, uploaded_at, transmission_status
            FROM imageuploads
            WHERE patient_id = ?
            ...
            cursor.execute("SELECT COUNT(*) FROM imageuploads WHERE patient_id = ?", (patient_id,))
...
def get_stats():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(DISTINCT patient_id) FROM imageuploads")
...
def get_image():
    ...
    try:
        ...
        cursor.execute("""
        SELECT imagedata, mime, filename
        FROM imageuploads
        WHERE id = ?
        """, (image_id,))
...
def get_patient_images():
    ...
    try:
        ...
        if include_thumbnails:
            query = """
            SELECT id, filename, mime, uploaded_at, thumbdata
            FROM imageuploads
            WHERE patient_id = ?
            ORDER BY uploaded_at DESC
            """
            ...
            query = """
            SELECT id, filename, mime, uploaded_at
            FROM imageuploads
            WHERE patient_id = ?
            ORDER BY uploaded_at DESC
            """
...
def list_patients():
    ...
    Query Parameters:
        keyword (str, optional): 搜尋關鍵字（姓名或 MRN）
    ...
    try:
        ...
        if keyword:
            ...
            cursor.execute("""
            SELECT id, name, mrn, gender, birthday, bloodtype, 
        ...
        for row in rows:
            patients.append({
                ...
                'gender': row[3],
                'birthday': row[4],
                'blood_type': row[5],  # ← 回傳時統一使用 blood_type
                'nationalid': row[6],
                'phone': row[7],
                'created_at': row[8]
            })
        
        print(f"✅ 回傳 {len(patients)} 位病患")
...
def create_patient():
    ...
    try:
        ...
        cursor.execute("""
        INSERT INTO patients (name, mrn, gender, birthday, bloodtype, nationalid, phone)
        VALUES (?, ?, ?, ?, ?, ?, ?)
...
def update_patient(patient_id):
    ...
    Request Body:
        要更新的欄位（使用 blood_type，會自動轉換為 bloodtype）
    ...
    try:
        ...
        field_mapping = {
            ...
            'gender': 'gender',
            'birthday': 'birthday',
            'blood_type': 'bloodtype',  # ← 關鍵：轉換欄位名稱
...
def delete_patient(patient_id):
    ...
    try:
        ...
        # 檢查是否有關聯的影像
        cursor.execute("SELECT COUNT(*) FROM images WHERE patient_id = ?", (patient_id,))
        image_count = cursor.fetchone()[0]
