...
from photo_storage import PhotoStorageManager
...
ca_port = None
# 監控相關全域變數
online_users = {}  # {user_id: {"username": str, "login_time": datetime, "role": str}}
encryption_events = []  # 最近的加密事件記錄
ca_events = []  # CA 驗證事件記錄
MAX_EVENTS = 100  # 最多保存的事件數量
photo_storage = PhotoStorageManager()
...
def register_with_remote_ca(entity, host, port):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ca_event = {
                    "event_id": len(ca_events) + 1,
                    "type": "REGISTER",
                    "timestamp": datetime.now().isoformat(),
                    "party_id": register_data['party_id'],
                    "ca_server": f"{host}:{port}",
                    "status": "SUCCESS",
                    "details": "憑證註冊成功"
                }
                if len(ca_events) >= MAX_EVENTS:
                    ca_events.pop(0)
                ca_events.append(ca_event)
            except:
                pass
...
def upload_image():
    """接收加密影像封包（含照片儲存機制）"""
    global encryption_events, ca_events
    ...
    try:
        ...
        original_filename = app_meta.get('filename', 'unknown.dat')
        
        # ============================================================
        # ⭐ 解密前：儲存加密照片
        # ============================================================
        encrypted_photo_path = None
        ciphertext_b64 = enc_data.get('ciphertext_b64', '')
        
        if ciphertext_b64:
            try:
                ciphertext = base64.b64decode(ciphertext_b64)
                encrypted_photo_path = photo_storage.save_encrypted_photo(
        ...
        try:
            decrypted_photo_path = photo_storage.save_decrypted_photo(
                plaintext, original_filename
            )
        except Exception as e:
        ...
        # ============================================================
        try:
            iv_b64 = enc_data.get('iv_b64', '')
            kem_ct_b64 = enc_data.get('kem_ciphertext_b64', '')
            
            iv_hex = base64.b64decode(iv_b64).hex() if iv_b64 else ''
            kem_ct_hex = base64.b64decode(kem_ct_b64).hex() if kem_ct_b64 else ''
            cipher_hex = base64.b64decode(ciphertext_b64).hex() if ciphertext_b64 else ''
            ...
            if uploader_id:
                try:
                    conn = connect_db()
                    cursor = conn.cursor()
                    cursor.execute("SELECT username FROM users WHERE id = ?", (uploader_id,))
                    row = cursor.fetchone()
                    if row:
                        uploader_name = row[0]
                    conn.close()
            ...
            encryption_event = {
                "event_id": len(encryption_events) + 1,
                "type": "DECRYPT",
                "direction": "UPLOAD",
                "timestamp": datetime.now().isoformat(),
                "user_id": uploader_id,
                "username": uploader_name,
                "filename": app_meta.get('filename'),
                "file_size": len(plaintext),
                "encryption_details": {
                    "kem_algorithm": enc_data.get('kem_alg', 'ML-KEM-1024'),
                    "aes_mode": "AES-256-GCM",
                    "kem_ciphertext_size": len(base64.b64decode(kem_ct_b64)) if kem_ct_b64 else 0,
                    "ciphertext_size": len(base64.b64decode(ciphertext_b64)) if ciphertext_b64 else 0,
                    "has_wrapped_key": 'wrapped_aes_key_b64' in enc_data,
                    "iv_hex": iv_hex,
                    "kem_ciphertext_hex": kem_ct_hex[:64] + "..." if len(kem_ct_hex) > 64 else kem_ct_hex,
                    "ciphertext_hex": cipher_hex[:64] + "..." if len(cipher_hex) > 64 else cipher_hex,
                    "encrypted_photo_path": encrypted_photo_path,
                    "decrypted_photo_path": decrypted_photo_path,
                },
            ...
            # ⭐ 新增：處理科別代碼
            clinical_dept_code = app_meta.get('clinical_dept_code')
            modality_dept_code = app_meta.get('modality_dept_code')

            clinical_dept_id = get_clinical_dept_id_by_code(clinical_dept_code)
            modality_dept_id = get_modality_dept_id_by_code(modality_dept_code)

            print(f"   臨床科別: {clinical_dept_code} -> ID {clinical_dept_id}")
            print(f"   影像科別: {modality_dept_code} -> ID {modality_dept_id}")
            ...
            image_id = insert_image_upload(
                ...
                mime=app_meta['mime'],
                clinical_dept_id=clinical_dept_id,      # ⭐ 新增
                modality_dept_id=modality_dept_id,      # ⭐ 新增
            )
            if hasattr(receiver, 'last_decryption_info') and receiver.last_decryption_info:
                encryption_event['encryption_details'].update({
                    "aes_key_hex": receiver.last_decryption_info.get('aes_key_hex', ''),
                    "shared_secret_hex": receiver.last_decryption_info.get('shared_secret_hex', ''),
                })
            
            if len(encryption_events) >= MAX_EVENTS:
                encryption_events.pop(0)
            encryption_events.append(encryption_event)
            
            ca_event = {
                "event_id": len(ca_events) + 1,
                "type": "VERIFY",
                "timestamp": datetime.now().isoformat(),
                "party_id": uploader_name or f"User #{uploader_id}",
                "ca_server": f"{ca_host}:{ca_port}",
                "status": "SUCCESS",
                "details": "上傳者憑證驗證成功"
            }
            if len(ca_events) >= MAX_EVENTS:
        ...
        # 顯示照片數量
        enc_count, dec_count = photo_storage.get_file_count()
        print(f"\\n✅ 上傳完成 - Image ID: {image_id}")
        print(f"   📁 加密照片資料夾: {enc_count} 個檔案")
        print(f"   📁 解密照片資料夾: {dec_count} 個檔案")
        print()
...
def request_image():
    ...
    try:
        ...
        try:
            ...
            print(f"📊 記錄加密事件 #{encryption_event['event_id']}")
            
        except Exception as e:
            print(f"⚠️ 記錄加密事件失敗: {e}")
...
def api_login():
    ...
    try:
        ...
        print(f" 線上用戶數: {len(online_users)}")
...
def api_logout():
    ...
    try:
        ...
        if user_id in online_users:
            removed_user = online_users.pop(user_id)
            print(f" 用戶 {removed_user.get('username')} 已登出，剩餘線上用戶: {len(online_users)}")
...
@app.route('/monitor/online_users', methods=['GET'])
...
@app.route('/monitor/encryption_events', methods=['GET'])
def get_encryption_events():
    '''獲取加密事件記錄'''
    limit = int(request.args.get('limit', 20))
    return jsonify({
        "status": "success",
        "events": encryption_events[-limit:],
        "total": len(encryption_events)
    })


@app.route('/monitor/ca_events', methods=['GET'])
def get_ca_events():
    '''獲取 CA 驗證事件記錄'''
    limit = int(request.args.get('limit', 20))
    return jsonify({
        "status": "success",
        "events": ca_events[-limit:],
        "total": len(ca_events)
    })
