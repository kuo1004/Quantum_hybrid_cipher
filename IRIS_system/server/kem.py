...
修正版：正確的資源管理 + 錯誤處理 + decap_secret 修正
"""
...
class SimpleKEM:
    ...
    def decap_secret(self, encapsulated_key):
        ...
        try:
            # ✅ 正確的方法：使用 context manager 並在內部設置私鑰
            with oqs.KeyEncapsulation(self.alg_name, secret_key=self.secret_key) as kem_receiver:
                shared_secret = kem_receiver.decap_secret(encapsulated_key)
                return shared_secret
                
        except Exception as e:
            # 如果 context manager 不支持 secret_key 參數，嘗試另一種方法
            try:
                kem_receiver = oqs.KeyEncapsulation(self.alg_name)
                # 手動導入私鑰
                ...
                raise RuntimeError(f"KEM 解封裝失敗: 方法1={e}, 方法2={e2}") from e2
