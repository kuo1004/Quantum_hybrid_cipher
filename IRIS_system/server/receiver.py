...
class Receiver:
    ...
    def receive_and_decrypt(self, transmission_package: dict, 
        ...
        try:
            ...
            if not self.verify_peer_certificate(sender_cert, ca_public_key_pem):
                print("❌ 錯誤: 傳送方憑證驗證失敗，可能來自不受信任的來源。")
                return False
            print("✅ 傳送方憑證驗證成功。")
            ...
            # 步驟 3: 使用本端的 KEM 私鑰解封裝 AES 金鑰
            kem = SimpleKEM(kem_alg)
            aes_key = kem.decapsecret(encapsulated_key)
            if not aes_key:
            ...
            sender_pub_pem = sender_cert['public_key_pem'].encode('utf-8')
            sender_public_key = serialization.load_pem_public_key(sender_pub_pem)

            if not verify_signature(sender_public_key, plaintext, signature_bytes):
                print("❌ 錯誤: 數位簽章驗證失敗！資料可能已被竄改或來源不符。")
                return False
            print("✅ 數位簽章驗證成功。")
...
def analyze_transmission_package(transmission_package: dict):
    """分析傳輸包內容"""
    print("\n📋 傳輸包分析:")
    print(f"   發送方: {transmission_package['sender_id']}")
    print(f"   接收方: {transmission_package['receiver_id']}")
    print(f"   時間戳: {time.ctime(transmission_package['timestamp'])}")

    encrypted_data = transmission_package["encrypted_data"]
    print(f"   影像模式: {encrypted_data['mode']}")
    print(f"   影像尺寸: {encrypted_data['size']}")
    print(f"   KEM算法: {encrypted_data['kem_alg']}")

    # 估算原始影像大小
    mode = encrypted_data['mode']
    size = encrypted_data['size']
    channels = len(mode) if mode in ['RGB', 'RGBA'] else 1
    estimated_size = size[0] * size[1] * channels
    print(f"   估算原始影像: ~{estimated_size} bytes")
